    ...
    for i in range(5):
        ...
        if choice == "h":
            ...
            if level == 1:
                ...
                data = data + [
                    [hand_value(players_hand), int(dealers_hand[0][:dealers_hand[0].index('-')])] + deck.negation()]
            # hit
...
def test_model(model_name, num_of_tests, fresh_deck, level, debug):
    ...
    for i in range(num_of_tests):
        ...
        # check for win/loss
        summ = hand_value(players_hand)
        print("Player's hand value is:",summ)
        bustcards = 0
        if summ <= 11:
            print("Chances of busting with the current hand are 0%")
        else:
            limit = 22 - summ
            nonbustcards = limit * 4
            bustcards = 52 - nonbustcards
            bustchance = bustcards / 52 * 100
            print("Chances of busting with the current hand are:", bustchance, "%")
        if summ == 21 and hand_value(dealers_hand) == 21:
            if debug:
                print("21 each.")
                print("TIE")
        ...
        for j in range(5):
            ...
            if choice == "h":
                ...
                if debug:
                    ...
                    print("Dealer: " + dealers_hand[0] + "   " + "x-x")
                    print("Player: " + players_hand_str)
                    print("Payer's hand value is:", summ)
                    bustcards = 0
                    if summ <= 11:
                        print("Chances of busting with the current hand are 0%")
                    else:
...
if __name__ == '__main__':
    #hand("blackjackmodel", 1, True )
    test_model("blackjackmodel", 1, True, 1, True)